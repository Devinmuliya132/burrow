...
- `utf8_valid_string` and `utf8_valid` skip a machine word at a time through a run of ASCII, which is Go's optimisation and most of what the function costs on real input. The word is built from separate byte loads and shifts, the way Go builds it, so there is no unaligned read and no dependence on byte order.
