...
TEST(version_string_matches_the_components) {
    char want[32];
...
TEST(version_number_matches_the_string) {
...
TEST(embedded_licence_carries_the_required_notices) {
...
int main(void) {
    RUN(version_string_matches_the_components);
    RUN(version_number_matches_the_string);
    RUN(provenance_is_recorded);
    RUN(embedded_licence_carries_the_required_notices);
    return harness_report("version");
}
